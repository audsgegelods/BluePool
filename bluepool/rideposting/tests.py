...
class TestRideRequestFunctionality(TestCase):
    ...
    def test_post_message(self):
        time = timezone.now()
        self.client.force_login(self.driver_user)
        self.client.post("/rideposting/ride/1", {"form_id": "send", "text": 'Hello World', 'user': self.driver_user, 'ride': Ride.objects.get(id=self.ride1.id)})
        self.assertTrue(Message.objects.exists())
        self.assertEqual(Message.objects.get(id=1).text, "Hello World")
        self.assertEqual(Message.objects.get(id=1).author, self.driver_user)